...
import { findMetric } from '@turingpointde/cvss.js/lib/util';
...
async function updateWhitelist() {
  ...
  class VulnerabilityReport {
    ...
    async chooseAction() {
      const spinner = prompts.spinner();
      spinner.start(this.getLoadingMessage());
      spinner.stop(await this.loadDependencyDetails());
      ...
      return `${chalk.bold('CVSS Score Reference: ')}
        ${[
          ...
          .map((message, index) => {
            ...
            colouredByScore(parseFloat(value), message);
          })
          .join(', ')}`;
    }
    ...
    ${description}
  ${chalk(
    `{bold CVSS Score:} ${colouredByScore(cvssScore)}`
...
${this.getVectorMetrics(vector)
  ...
  .join('\n')}`;
    ...
    async loadDependencyDetails() {
      ...
      const dependencyVersion = this.dep.coordinates.substring(8);
      try {
        const usages = await this.loadNpmPackages(dependencyVersion);
        if (!usages)
          return `${this.getHeader()}\nFailed to load dependencies for ${dependencyVersion}`;
        const dependencyTree = treeify.asTree(
          this.buildDependenciesFromUsages(dependencyVersion, usages),
          true,
          true
        );
        return `${this.getHeader()}\n${chalk.bold(
