...
export function getBundleInput(cwd) {
    ...
    if (inputsArg) {
        ...
        return inputs.map((input) => {
            ...
            if (!fs.existsSync(result))
                throw new Error(`"${pkgFile.entries ? 'entries' : 'entry'}" value ${input} does not refer to a valid file (${result})`);
            return result;
        });
    }
    ...
    if (fs.existsSync(tsxEntry))
        return [tsxEntry];
    throw new Error(`Package entries/entry key was not set and could not find a valid "index.ts" or "index.tsx" in "${path.join(cwd, 'src')}"`);
}
...
export function loadRollupConfig({ packagePath, cwd, bundleEntries, rollupWarnings, shouldBundleDependencies, verbose, }) {
    ...
    const rollupConfig = {
        ...
        onwarn: (warning) => {
            ...
            if (warning.code === 'UNRESOLVED_IMPORT') {
                rollupWarnings.push(`${warning.message}
