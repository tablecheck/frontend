  ...
  for (let i = 0; i < foundVulnerabilities; i += 1) {
        ...
        try {
          if (cvssVector) {
            ...
            Object.keys(vector.metrics).forEach((metricKey) => {
              ...
              if (!valueDefinition) {
                console.log(
                  colouredByScore(99, chalk`{bold ${fullName}:} ${value}`)
                );
              } else {
    ...
    if (dep.coordinates.match(/^pkg:npm\//)) {
      ...
      try {
        ...
        >;
        const buildDependencies = (dependencies: RecursiveDeps) => {
          ...
          return Object.keys(dependencies).reduce((tree, name) => {
            ...
            if (childDeps) tree[displayName] = buildDependencies(childDeps);
            return tree;
          }, {} as any);
        };
        console.log(
          treeify.asTree(
            buildDependencies(JSON.parse(usages).dependencies),
            true,
            true
          )
        );
