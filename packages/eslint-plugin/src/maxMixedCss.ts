...
export const messageId = 'max-mixed-css' as const;

function checkImport(variables: TSESLint.Scope.Variable[], importName: string) {
  ...
  const declarationVar = identifiers.find(
    (ident) =>
      ...
      (ident.parent.parent as TSESTree.ImportDeclaration)?.source.value.match(
        /^@emotion\//
      )
  );
...
export const maxMixedCss: TSESLint.RuleModule<typeof messageId, number[]> = {
  meta: {
    type: 'suggestion',
    docs: {
      description:
  ...
  create: (context) => {
    ...
    let cssCount = 0;
    let jsxCount = 0;
    let reportNode: TSESTree.TemplateLiteral;
    const countedJsxNodes: TSESTree.JSXElement[] = [];
    ...
    function validImport(node: TSESTree.TemplateLiteral) {
      const { variables } = getModuleScope();
      const { tag } = node.parent as any;
      if (!tag) return false;
        ...
        for (let l = 0; l < quasiLines.length; l += 1) {
          ...
          if (remainingStatements >= 0) {
            ...
            loc = {
              ...
              end: { line: quasi.loc.start.line + l + 1, column: Infinity }
            };
          }
        }
      }
    ...
    return {
      JSXElement(node) {
        ...
        while (testParent.parent && testParent.parent.type !== 'Program') {
          ...
          testParent = testParent.parent as any;
        }
