...
export async function buildPackage({ cwd, outputBundle, verbose, }) {
    const packagePath = path.join(cwd, 'package.json');
    prompts.intro(`Build ${(await import(packagePath)).name ||
        cwd.trim() ||
        'library'}`);
    ...
    try {
        ...
        const bundle = await rollup.rollup(loadRollupConfig({
            packagePath,
            cwd,
            bundleEntries,
            rollupWarnings,
        ...
        if (outputBundle) {
            ...
            const bundleEs5WithDeps = await rollup.rollup(loadRollupConfig({
                packagePath,
                cwd,
                bundleEntries,
                rollupWarnings,
        ...
        if (baseUrl) {
            fs.readdirSync(path.join(cwd, baseUrl), {
                ...
                if (dirent.isDirectory())
                    ...
                    pathKeys.push(dirent.name.split('.').slice(0, -1).join('.'));
            });
        }
        const pathMatchers = pathKeys.map((key) => (relativePath) => {
            if (!relativePath.match(new RegExp(`^${key.replace(/[*/]*$/gi, '.*')}`, 'ig')))
                return false;
            return `./${relativePath}`;
        });
        const tsDefinitionFiles = await glob(path.join(libCwd, '**/*.d.ts'));
        let transformFiles = bundleEntries.map((bundleEntry) => path.join(libCwd, path
            .relative(path.join(cwd, baseUrl), bundleEntry)
            .replace(/\.tsx?$/, '.d.ts')));
        while (transformFiles.length) {
            // eslint-disable-next-line no-await-in-loop
            const imports = await Promise.all(transformFiles.map((filepath) => writeTypes(libCwd, tsDefinitionFiles, pathMatchers, filepath)));
            transformFiles = imports
                ...
                if (fs.existsSync(filepath))
                    return filepath;
                throw new Error(`Cannot find file for ${filepath}, check that it is not using any private variables like CONFIG`);
            })
                .reduce((r, filepath) => {
                if (r.includes(filepath))
                    return r;
        ...
        if (error.frame && error.loc) {
            prompts.note(`${error.message}\n${error.frame}`, `Rollup error: ./${path.relative(cwd, error.loc.file)}@${error.loc.line}:${error.loc.column}`);
        }
        else {
